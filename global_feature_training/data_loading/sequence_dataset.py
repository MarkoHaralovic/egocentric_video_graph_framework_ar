...
class SequenceDataset(Dataset):
   ...
   def __getitem__(self, idx):
      ...
      with h5py.File(self.h5_paths[file_idx], 'r') as f:
         if self.load_visual:
            output['visual_features'] = torch.from_numpy(f['visual_features'][block_idx].astype(np.float32))
         if self.load_text:
            output['text_features'] = torch.from_numpy(f['text_features'][block_idx].astype(np.float32))
...
def feature_collate_fn(batch):
   ...
   for key in ['visual_features', 'text_features']:
      if key in batch[0]:
         ...
         output[key] = torch.stack(feats) if all(f.shape == feats[0].shape for f in feats) else feats
