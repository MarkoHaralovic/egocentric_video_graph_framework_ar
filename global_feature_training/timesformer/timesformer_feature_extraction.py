...
def get_timesformer_visual_backbone(
    img_size=224,
    num_classes=600,
    num_frames=8,
...
def process_folder(
    ...
    clips = [
        clip
        for clip in os.listdir(input_folder_path)
        if os.path.isdir(os.path.join(input_folder_path, clip))
    ]
    ...
    for clip in tqdm.tqdm(clips, desc=f"Extracting features from {input_folder_path}"):
        ...
        for activity_block in activity_blocks_ids:
            ...
            sampled_frames, sampled_actions, sampled_gazes = sample_frames_and_labels(
                frames, actions, gazes, num_frames
            )

            image_features = get_visual_features(
                vision_backbone, sampled_frames, device=device
            )
            text_features = get_text_features(
                text_backbone, tokenizer, sampled_actions, device
            )
...
def main():
    ...
    tokenizer, text_backbone, _ = get_clip_text_encoder(
        clip_model_name, cache_dir=MODEL_CACHE_DIR
    )
