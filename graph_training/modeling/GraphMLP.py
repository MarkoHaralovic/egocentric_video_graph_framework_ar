...
class ActionGraphEmbedding(nn.Module):
   def __init__(
      self,
      num_verbs,num_objects,num_rels,num_attrs,clip_dim,clip_emb_dim,obj_feat_dim,
   ...
   def forward(self, g: Dict[str, torch.Tensor]) -> torch.Tensor:
      ...
      if aux_idx is None or aux_idx.numel() == 0:
         aux_tokens = torch.zeros((0, self.verb_emb.emb.embedding_dim), device=self.device)
      else:
      ...
      if self.use_triplets:
         ...
         if trip.shape[0] == 0:
               trip_tokens = torch.zeros((0, self.verb_emb.emb.embedding_dim), device=self.device)
         else:
               t = torch.cat(
                  [
...
class GraphMLP(nn.Module):
   def __init__(
      ...
      if self.fc_layers_num == 1 :
         ...
         for _ in range(self.fc_layers_num - 1):
            layers.append(nn.Linear(self.final_graph_emb_dim, self.final_graph_emb_dim))
   ...
   def forward(self, sequence_graphs : List):
      ...
      for _sequence_graphs in sequence_graphs:
         graph_embs = [self.action_graph_embedder(graph.to_easg_tensors()).to(self.device) for graph in _sequence_graphs.values()] 
