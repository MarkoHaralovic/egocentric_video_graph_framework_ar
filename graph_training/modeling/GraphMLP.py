...
class ActionGraphEmbedding(nn.Module):
   def __init__(
      ...
      k_obj=4,k_aux=2,k_trip=2,use_triplets=True,
   ):
      super().__init__()
   ...
   def forward(self, g: Dict[str, torch.Tensor]) -> torch.Tensor:
      ...
      if aux_idx is None or aux_idx.numel() == 0:
         aux_tokens = torch.zeros((0, v.shape[0]), device=device)
      else:
      ...
      attr_sum = attr_vecs.sum(dim=0) if attr_vecs.shape[0] > 0 else torch.zeros(attr_vecs.shape[1], device=device)
      attr_emb = self.attr_proj(torch.cat([attr_sum, torch.log1p(attr_sum)], dim=0))

      rels_vecs = g["rels_vecs"]
      rel_sum = rels_vecs.sum(dim=0) if rels_vecs.shape[0] > 0 else torch.zeros(rels_vecs.shape[1], device=device)
      rel_emb = self.rel_proj(torch.cat([rel_sum, torch.log1p(rel_sum)], dim=0))

      if self.use_triplets:
         trip = g["triplets"]
         if trip.shape[0] == 0:
               trip_tokens = torch.zeros((0, v.shape[0]), device=device)
         else:
               t = torch.cat(
                  [
...
class GraphMLP(nn.Module):
   def __init__(self,num_graphs,num_verbs,num_objects,num_rels,num_attrs,clip_dim,obj_feat_dim, n_classes, fc_layers_num):
      super().__init__()
      self.num_graphs = num_graphs
      self.action_graph_embedder = ActionGraphEmbedding(num_verbs,num_objects,num_rels,num_attrs,clip_dim,obj_feat_dim)
