...
class FullActionGraph(BaseGraph):
   ...
   def create_graph(
      ...
      if direct_object is not None : 
         direct_object_id = self.objs[to_singular(objects_atr_map[direct_object]["base_object"])] 
      else:
      ...
      aux_direct_object_ids = [self.objs[to_singular(objects_atr_map[aux_direct_object]["base_object"])] for aux_direct_object in aux_direct_objects]  if aux_direct_objects is not None else None
      ...
      for (obj_idx, orig_name, attributes) in obj_data:
         ...
         if direct_object_id is not None and obj_idx == direct_object_id:
            self.edges.append(self.rel_edge(verb_id, node_id, rel_idx=self.rels["direct_object"]))
      ...
      if aux_verb_idxs and aux_direct_objects_map:
         for aux_verb, aux_verb_node_id in aux_verb_node_map.items():
            if aux_direct_objects_map[aux_verb]:
               ...
               aux_obj_idx = self.objs[to_singular(objects_atr_map[aux_obj]["base_object"])]
   ...
   def to_easg_tensors(self) -> Dict[str, torch.Tensor]:
      ...
      for obj_node in obj_nodes:
         obj_row = obj_nodes.index(obj_node)
         if obj_node.attr:
            attrs_list = obj_node.attr if isinstance(obj_node.attr, list) else [obj_node.attr]
