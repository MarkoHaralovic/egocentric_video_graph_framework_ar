...
N_FRAMES_FOR_ACTIVITY = (
    10  # number of consecutive action frames to be sent for activity annotation
)
...
# data location
INPUT_DATA_FOLDER = (
    "/deepstore/datasets/dmb/ComputerVision/information_retrieval/AriaEA"
)
...
def load_llava_model(
    ...
    processor = LlavaNextProcessor.from_pretrained(
        model_name, cache_dir=MODEL_CACHE_DIR, local_files_only=True, use_fast=True
    )
    model.eval()
...
def recognize_action_single_frame(
    model, processor, image_np, image_size=IMAGE_SIZE_VLM_INPUT
):
    """
...
def recognize_action_local_window(
    model, processor, images_np, image_size=IMAGE_SIZE_VLM_INPUT
):
    """
...
def recognize_activity(
    model, processor, images_np, image_size=IMAGE_SIZE_VLM_INPUT, actions=None
):
    """
    ...
    for img in images:
        img.thumbnail(
            (activity_image_size, activity_image_size), Image.Resampling.LANCZOS
        )
...
def action_annotate(
    ...
    processor,
    images,
    local_window=LOCAL_WINDOW_FOR_ACTION_SIZE,
    ...
    for i in tqdm.tqdm(range(len(images)), desc="Action annotating clip"):
        ...
        if LOCAL_WINDOW_FOR_ACTION_SIZE == 1:
            result = recognize_action_single_frame(
                model, processor, images_for_act[0], image_size
            )
        else:
            result = recognize_action_local_window(
                model, processor, images_for_act, image_size
            )
        ...
        while redo:
            ...
            try:
                result = recognize_activity(
                    model, processor, act_images, act_image_size, actions=actions
                )
                redo = False
            except torch.cuda.OutOfMemoryError:
                torch.cuda.empty_cache()
...
def save_images_annotations(
    images_np, output_path, actions, activities, N_frames_for_activity, gazes=None
):
